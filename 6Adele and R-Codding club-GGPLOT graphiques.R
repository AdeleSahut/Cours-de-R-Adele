...
head(gridIC)
summary(gridIC)
Graphline<-ggplot(gridIC, aes(Years,Abundance, colour=Country.list))+ 
  geom_point()+
  ggtitle("Figure 1: Evolution de l'abondance de griffons en Italie et en Croatie")+
  xlim(1985,2010)+
  geom_point(size = 4)+ 
...
Graphbox<-ggplot(gridIC,aes(Country.list,Abundance))+
  geom_boxplot(aes(fill = Country.list)) +
  ggtitle("Figure 2: Abondance de Griffon en Italie et en Croatie")+
...
##Barplot
 ...
 a <- unique(select(rich,Country.list, richesse))
  ...
  Graphbar<- ggplot(a, aes(x = Country.list, y = richesse)) + 
     geom_bar(position = position_dodge(), , stat = "identity",colour = "black", fill = "#00868B")+
   ggtitle("Figure 3: Richesse specifique du Canada, des Etats Unis et du Bresil")
   
###Resultats
  grid.arrange(Graphline,Graphbox,Graphbar)
